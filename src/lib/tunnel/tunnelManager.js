...
const WORKER_URL = process.env.TUNNEL_WORKER_URL || "https://llm-gateway.com";
const MACHINE_ID_SALT = "llm-gateway-tunnel-salt";
  ...
  try {
    if (isCloudflaredRunning()) {
      ...
      if (existing?.tunnelUrl && await probeUrlAlive(existing.tunnelUrl)) {
        const publicUrl = `https://r${existing.shortId}.llm-gateway.com`;
    ...
    const publicUrl = `https://r${shortId}.llm-gateway.com`;
    ...
    // Then verify public URL (DNS propagated through llm-gateway.com worker)
...
export async function getTunnelStatus() {
  ...
  const publicUrl = shortId ? `https://r${shortId}.llm-gateway.com` : "";
