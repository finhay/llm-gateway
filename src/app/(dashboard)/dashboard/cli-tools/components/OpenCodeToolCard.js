...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("llm-gateway/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("llm-gateway/", ""));
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_llm-gateway" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasLlmGateway) return "not_configured";
    const url = status.config?.provider?.["llm-gateway"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `llm-gateway/${activeModelToShow}`,
        agent: {
          explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["llm-gateway"]?.options?.baseURL && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasLlmGateway} loading={restoring}>
