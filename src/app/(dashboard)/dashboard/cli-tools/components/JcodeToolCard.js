...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasLlmGateway) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["llm-gateway"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["llm-gateway"];
      if (provider) {
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse = selectedApiKey?.trim()
        ...
        || (!cloudEnabled ? "sk_llm-gateway" : null);
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_llm-gateway" : "<API_KEY_FROM_DASHBOARD>");

    const configToml = `[providers.llm-gateway]
...
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-llm-gateway.env",
        content: envContent,
      },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if llm-gateway is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["llm-gateway"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile llm-gateway</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile llm-gateway --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasLlmGateway} loading={restoring}>
