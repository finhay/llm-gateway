...
const hasLlmGatewayConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["llm-gateway"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["llm-gateway"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasLlmGateway: hasLlmGatewayConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("llm-gateway/") ? config.model.replace(/^llm-gateway\//, "") : null,
...
// POST - Apply LLM Gateway as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_llm-gateway";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing llm-gateway provider entry and its models
    const existingProvider = config.provider["llm-gateway"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["llm-gateway"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `llm-gateway/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `llm-gateway/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("llm-gateway/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove LLM Gateway provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["llm-gateway"]?.models) {
      delete config.provider["llm-gateway"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["llm-gateway"].models).length === 0) {
        delete config.provider["llm-gateway"];
        if (config.model?.startsWith("llm-gateway/")) delete config.model;
      } else if (config.model === `llm-gateway/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["llm-gateway"].models);
        config.model = `llm-gateway/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire llm-gateway provider
      if (config.provider) delete config.provider["llm-gateway"];
      if (config.model?.startsWith("llm-gateway/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("llm-gateway/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "LLM Gateway settings removed from OpenCode",
    });
  } catch (error) {
