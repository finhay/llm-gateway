...
// Check if settings has LLM Gateway config
const hasLlmGatewayConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["llm-gateway"];
};

// Read per-agent models.json and return current model id (without "llm-gateway/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["llm-gateway"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasLlmGateway: hasLlmGatewayConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["llm-gateway"] = {
    baseUrl,
...
// POST - Update LLM Gateway settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `llm-gateway/${model}`;

    // Remove all old llm-gateway/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("llm-gateway/"))
    ...
    // Add fresh llm-gateway models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`llm-gateway/${m}`] = {};
    });

    // Remove old llm-gateway model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (agent.model?.startsWith("llm-gateway/")) {
    ...
    // Update models.providers.llm-gateway with all models
    settings.models.providers["llm-gateway"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `llm-gateway/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove LLM Gateway settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove LLM Gateway from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["llm-gateway"];
    ...
    // Remove llm-gateway models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("llm-gateway/"));
    ...
    // Reset agents.defaults.model.primary if it uses llm-gateway
    if (settings.agents?.defaults?.model?.primary?.startsWith("llm-gateway/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "LLM Gateway settings removed successfully",
    });
  } catch (error) {
