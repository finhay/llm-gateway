...
const hasLlmGatewayConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["llm-gateway"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("llm-gateway");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasLlmGateway: hasLlmGatewayConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "LLM Gateway", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "LLM Gateway settings removed from Kilo Code" });
  } catch (error) {
