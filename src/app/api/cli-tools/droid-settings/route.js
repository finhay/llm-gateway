...
// Check if settings has LLM Gateway customModels
const hasLlmGatewayConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:LLM Gateway"));
};
...
// POST - Update LLM Gateway customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing LLM Gateway configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:LLM Gateway"));
...
// DELETE - Remove LLM Gateway customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove LLM Gateway customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:LLM Gateway"));
