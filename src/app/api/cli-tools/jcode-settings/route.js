...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-llm-gateway.env");
};
...
const hasLlmGatewayConfig = (config) => {
  ...
  if (providers["llm-gateway"]) return true;
...
export async function GET() {
  ...
  const hasLlmGateway = hasLlmGatewayConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile llm-gateway",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["llm-gateway"];
    ...
    return NextResponse.json({
      success: true,
      message: "llm-gateway configuration removed from jcode",
    });
  } catch (error) {
