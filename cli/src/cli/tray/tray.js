...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `LLM Gateway (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[llm-gateway] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[llm-gateway] tray init error: ${err.message}\n`);
    return null;
  }
}
