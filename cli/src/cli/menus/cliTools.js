...
async function buildCodexHeader() {
  ...
  const { installed, hasLlmGateway, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasLlmGateway, settings } = result.data;
  ...
  // Extract LLM Gateway custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:LLM Gateway-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract LLM Gateway provider config
  const provider = settings?.models?.providers?.["llm-gateway"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("llm-gateway/") ? primary.replace("llm-gateway/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasLlmGateway, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use LLM Gateway\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
