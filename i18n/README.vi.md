...
<div align="center">
  <img src="../images/llm-gateway.png?1" alt="Bảng điều khiển LLM Gateway" width="800"/>
  
  # LLM Gateway - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [![npm](https://img.shields.io/npm/v/llm-gateway.svg)](https://www.npmjs.com/package/llm-gateway)
  [![Downloads](https://img.shields.io/npm/dm/llm-gateway.svg)](https://www.npmjs.com/package/llm-gateway)
  [![License](https://img.shields.io/npm/l/llm-gateway.svg)](https://github.com/decolua/9router/blob/main/LICENSE)
  
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://llm-gateway.com)
</div>

---

## 🤔 Tại sao chọn LLM Gateway?
...
**LLM Gateway giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`llm-gateway-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - LLM Gateway + Claude Code MIỄN PHÍ
  
[![Thiết lập LLM Gateway + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập LLM Gateway
...
LLM Gateway hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → LLM Gateway dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân LLM Gateway **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của LLM Gateway

**Thực tế Thanh toán LLM Gateway:**

✅ **Phần mềm LLM Gateway = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **LLM Gateway không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, LLM Gateway chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị LLM Gateway tính phí không?</b></summary>

**Không.** LLM Gateway là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, LLM Gateway chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân LLM Gateway** → **Không bao giờ tính phí bất cứ thứ gì, ever**

LLM Gateway là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
LLM Gateway chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và LLM Gateway giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải LLM Gateway.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - LLM Gateway giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của LLM Gateway ngăn chặn các khoản phí bất ngờ:

**Kịch bản:** Bạn đang trong giai đoạn code nước rút và vượt qua các hạn mức

**Không có LLM Gateway:**
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và LLM Gateway sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. LLM Gateway theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển llm-gateway]
...
```json
{
  ...
  "anthropic_api_key": "your-llm-gateway-api-key"
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": {
        "primary": "llm-gateway/if/glm-4.7"
      }
    }
  },
  "models": {
...
> **Lưu ý:** OpenClaw chỉ hoạt động với LLM Gateway cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
export NEXT_PUBLIC_CLOUD_URL="https://llm-gateway.com"
...
# Hoặc sử dụng PM2
npm install -g pm2
pm2 start --name llm-gateway -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name llm-gateway \
  -p 20128:20128 \
  --env-file /root/dev/llm-gateway/.env \
  -v llm-gateway-data:/app/data \
  -v llm-gateway-usage:/root/.llm-gateway \
  llm-gateway
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.llm-gateway/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.llm-gateway` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://llm-gateway.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://llm-gateway.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.llm-gateway/usage.json` và `~/.llm-gateway/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.llm-gateway` và độc lập với `DATA_DIR`.
...
**Token OAuth hết hạn**
- Tự động làm mới bởi LLM Gateway
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://llm-gateway.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://llm-gateway.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime LLM Gateway cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://llm-gateway.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://llm-gateway.com/v1/chat/completions`):
...
Cảm ơn tất cả những người đã đóng góp giúp LLM Gateway tốt hơn!
