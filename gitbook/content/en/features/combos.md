...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that LLM Gateway tries in order.
...
LLM Gateway automatically tries each model in sequence until one succeeds.
...
When limit reached, LLM Gateway skips paid models and uses free tier only.
