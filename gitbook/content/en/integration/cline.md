...
Integrate LLM Gateway with Cline VSCode extension to route your AI requests through LLM Gateway's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- LLM Gateway running locally or cloud endpoint configured
...
Set the base URL to your LLM Gateway endpoint:

**For Local LLM Gateway:**
```
http://localhost:20128/v1
```

**For Cloud LLM Gateway:**
```
https://llm-gateway.com
```

**Steps:**
1. In the **Base URL** field, enter your LLM Gateway endpoint
...
3. The key should start with `sk-llm-gateway-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your LLM Gateway configuration
...
4. Cline will use LLM Gateway to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using LLM Gateway
...
3. Get AI-powered explanations through LLM Gateway

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use LLM Gateway to understand context and make changes
...
1. Verify LLM Gateway is running: `curl http://localhost:20128/health`
...
2. Verify your LLM Gateway instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check LLM Gateway logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use LLM Gateway cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://llm-gateway.com`
2. Make sure you have configured your API key in the LLM Gateway cloud dashboard
...
## Integration with LLM Gateway Features

### Model Routing

LLM Gateway automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, LLM Gateway automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through LLM Gateway dashboard:
- Total requests
- Token usage
- Cost per model
