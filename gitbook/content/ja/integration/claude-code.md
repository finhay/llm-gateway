...
LLM GatewayをClaude Code CLIと統合し、AnthropicのAPIリクエストをLLM Gatewayのインテリジェントルーティングシステム経由でルーティングします。
...
Claude CodeはLLM Gatewayモデルにマッピングされる以下のモデルエイリアスをサポート:
...
1. モデル名がLLM Gatewayの設定と一致しているか確認
2. LLM Gatewayダッシュボードでプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにLLM Gatewayクラウドエンドポイントを使用するには:

```bash
export ANTHROPIC_BASE_URL="https://llm-gateway.com"
```

LLM GatewayクラウドダッシュボードでAPIキーが設定されていることを確認してください。
