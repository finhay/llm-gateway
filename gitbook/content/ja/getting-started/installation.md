...
トラブルシューティングのヒント付きのLLM Gateway詳細インストールガイド。
...
どこからでも使用できるようにLLM Gatewayをグローバルインストール:
...
**利点:**
- ✅ どのディレクトリからでも実行
- ✅ シンプルなコマンド: `llm-gateway`
- ✅ `npm update -g llm-gateway` で自動更新
...
3. `~/.llm-gateway` にデータディレクトリが作成される
4. APIキーが自動生成される
