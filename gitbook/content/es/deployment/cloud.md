...
Despliega LLM Gateway en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.llm-gateway` | Ruta de almacenamiento de la base de datos |
...
Crea `/etc/nginx/sites-available/llm-gateway`:
...
# Si NO usas proxy reverso, permite los puertos de LLM Gateway
...
```bash
# Verificar si LLM Gateway está corriendo
pm2 status
