...
Integra LLM Gateway con la extensión Cline de VSCode para enrutar tus solicitudes de IA a través del sistema de enrutamiento inteligente de LLM Gateway.

## Requisitos previos

- Visual Studio Code instalado
- Extensión Cline instalada desde el marketplace de VSCode
- LLM Gateway ejecutándose localmente o endpoint en la nube configurado
...
Establece la URL base a tu endpoint de LLM Gateway:

**Para LLM Gateway local:**
```
http://localhost:20128/v1
```

**Para LLM Gateway en la nube:**
```
https://llm-gateway.com
```

**Pasos:**
1. En el campo **Base URL**, ingresa tu endpoint de LLM Gateway
...
3. La key debe comenzar con `sk-llm-gateway-`

### 5. Seleccionar modelo

1. En el dropdown **Model**, puedes:
   - Seleccionar de los modelos disponibles (si Cline los auto-detecta)
   - Ingresar manualmente el nombre del modelo desde tu configuración de LLM Gateway
...
4. Cline usará LLM Gateway para procesar tu solicitud

### Generación de código

1. Pide a Cline que genere código: "Create a React component for a login form"
2. Cline generará código usando LLM Gateway
...
3. Obtén explicaciones potenciadas por IA a través de LLM Gateway

### Operaciones con archivos

1. Pide a Cline que cree, modifique o elimine archivos
2. Cline usará LLM Gateway para entender el contexto y hacer cambios
...
1. Verifica que LLM Gateway esté corriendo: `curl http://localhost:20128/health`
...
2. Verifica que tu instancia de LLM Gateway esté ejecutándose y saludable
3. Intenta recargar la ventana de VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Revisa los logs de LLM Gateway para cualquier error

## Configuración avanzada

### Usar endpoint en la nube

Para usar el endpoint en la nube de LLM Gateway en lugar de localhost:

1. En la configuración de Cline, establece Base URL a: `https://llm-gateway.com`
2. Asegúrate de haber configurado tu API key en el dashboard en la nube de LLM Gateway
...
## Integración con características de LLM Gateway

### Enrutamiento de modelos

LLM Gateway enruta automáticamente tus solicitudes al mejor proveedor disponible según:
...
Si un proveedor falla, LLM Gateway automáticamente cambia a proveedores alternativos configurados en tu dashboard.

### Seguimiento de uso

Monitorea tu uso de Cline a través del dashboard de LLM Gateway:
