...
Problemas comunes y soluciones al usar LLM Gateway.
...
1. **Refresh automático (por defecto):**
   LLM Gateway refresca automáticamente los tokens. Espera 30 segundos y reintenta.
...
**Causas:**
- LLM Gateway no está ejecutándose
- Puerto 20128 bloqueado
- Firewall bloqueando la conexión

**Soluciones:**

1. **Inicia LLM Gateway:**
   ```bash
   llm-gateway
   ```
...
4. **Usa el endpoint en la nube:**
   ...
   ```
   Endpoint: https://llm-gateway.com/v1
   ```
...
1. **Verifica si LLM Gateway está ejecutándose:**
   ```bash
   # Verifica el proceso
