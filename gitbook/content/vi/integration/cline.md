...
Tích hợp LLM Gateway với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của LLM Gateway.
...
Đặt base URL tới endpoint LLM Gateway:

**Cho LLM Gateway cục bộ:**
```
http://localhost:20128/v1
```

**Cho LLM Gateway cloud:**
```
https://llm-gateway.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint LLM Gateway
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key LLM Gateway của bạn
2. Bạn có thể tìm API key trong LLM Gateway dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-llm-gateway-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình LLM Gateway
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng LLM Gateway để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua LLM Gateway
...
3. Nhận giải thích AI qua LLM Gateway

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng LLM Gateway để hiểu context và thực hiện thay đổi
...
2. Xác minh LLM Gateway instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs LLM Gateway để xem lỗi
...
1. Trong Cline settings, đặt Base URL: `https://llm-gateway.com`
2. Đảm bảo bạn đã cấu hình API key trong LLM Gateway cloud dashboard
...
## Tích hợp với Tính năng LLM Gateway

### Định tuyến Model

LLM Gateway tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, LLM Gateway tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua LLM Gateway dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
