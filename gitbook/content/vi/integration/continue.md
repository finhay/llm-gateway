...
Tích hợp LLM Gateway với extension Continue để mang trợ lý AI trực tiếp vào Visual Studio Code.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Continue đã cài đặt từ VSCode marketplace
- LLM Gateway API key từ [dashboard](https://llm-gateway.com/dashboard)
- LLM Gateway đang chạy (cục bộ hoặc cloud)
...
### 2. Thêm Cấu hình Model LLM Gateway
...
3. Chọn model LLM Gateway ưa thích
...
- Kiểm tra LLM Gateway đang chạy: `curl http://localhost:20128/health`
...
- Kiểm tra độ trễ network đến LLM Gateway
