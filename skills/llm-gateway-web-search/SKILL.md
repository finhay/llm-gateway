---
name: llm-gateway-web-search
description: Web search via LLM Gateway /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---
...
```bash
curl $LLM_GATEWAY_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$LLM_GATEWAY_URL/v1/models/info?id=tavily/search"
```
...
`POST $LLM_GATEWAY_URL/v1/search`
...
```bash
curl -X POST $LLM_GATEWAY_URL/v1/search \
  -H "Authorization: Bearer $LLM_GATEWAY_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily/search","query":"LLM Gateway open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.LLM_GATEWAY_URL}/v1/search`, {
  method: "POST",
