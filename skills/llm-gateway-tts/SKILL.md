---
name: llm-gateway-tts
description: Text-to-speech via LLM Gateway /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# LLM Gateway — Text-to-Speech
...
```bash
# 1) List models
curl $LLM_GATEWAY_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$LLM_GATEWAY_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$LLM_GATEWAY_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $LLM_GATEWAY_URL/v1/audio/speech`
...
```bash
curl -X POST "$LLM_GATEWAY_URL/v1/audio/speech" \
...
const r = await fetch(`${process.env.LLM_GATEWAY_URL}/v1/audio/speech`, {
  method: "POST",
