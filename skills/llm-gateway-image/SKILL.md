...
description: Generate images via LLM Gateway /v1/images/generations using OpenAI / Gemini Imagen / DALL-E / FLUX / MiniMax / SDWebUI / ComfyUI / Codex models. Use when the user wants to create, generate, draw, or render an image, picture, or text-to-image (txt2img).
---
...
```bash
curl $LLM_GATEWAY_URL/v1/models/image | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$LLM_GATEWAY_URL/v1/models/info?id=openai/dall-e-3"
```

## Endpoint

`POST $LLM_GATEWAY_URL/v1/images/generations`
...
```bash
curl -X POST "$LLM_GATEWAY_URL/v1/images/generations?response_format=binary" \
...
```js
const r = await fetch(`${process.env.LLM_GATEWAY_URL}/v1/images/generations`, {
  method: "POST",
